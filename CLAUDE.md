...
**Последний релиз: v0.6.3.**

**v0.6.3 (релиз):** «Сквозные исправления». Пер-встречный словарь замен «как
написано → как надо» (фамилии/компании/термины), применяется ко всей расшифровке
и всем ИИ-отчётам сразу и к новым генерациям. Чисто фронтенд: `src/fixes.ts`
(`applyFixes` — целое слово, регистронезависимо, Unicode; localStorage
`3uxo.fixes.<id>`), карточка в `MeetingView` (`applyFixList` → `saveTranscript`/
`saveReport`, `reportsToken` → перезагрузка `AiPanel`), авто-применение в
`AiPanel.run()` к новым отчётам, чекбокс `fixEverywhere` в настройках (дефолт вкл).

**v0.6.2 (релиз):** центрированная SVG-иконка play/pause (символ ▶ был смещён).

**v0.6.1 (релиз, PR #34):** правка результатов (см. ниже).

Хронология 0.5.x: 0.5.0 (сборка упала, E0716, не
