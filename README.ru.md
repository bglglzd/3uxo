...
- **Родной на Windows и Mac** — в том числе Mac на Intel и Apple Silicon; на macOS Auris выглядит как приложение Apple: полупрозрачная боковая панель, шрифт SF, строка меню, сочетания с ⌘.
...
1. Скачайте Auris из [последнего релиза](https://github.com/bglglzd/auris/releases/latest):
   - **Windows** — `Auris_x.y.z_x64-setup.exe`, запустите установщик.
   - **Mac на Apple Silicon** (M1 и новее) — `Auris_x.y.z_aarch64.dmg`; **Mac на Intel** — `Auris_x.y.z_x64.dmg`. Перетащите Auris в «Программы». См. [первый запуск на macOS](#первый-запуск-на-macos).
...
1. Откройте «Программы», **правый клик по Auris → Открыть**, затем ещё раз **Открыть**. (Или: *Системные настройки → Конфиденциальность и безопасность* → **Всё равно открыть**.) Нужно один раз.
2. **Микрофон** — macOS спросит при первой записи, разрешите.
3. **Голоса собеседников** — системный звук записывается через ScreenCaptureKit, для этого нужно *Системные настройки → Конфиденциальность и безопасность → Запись экрана и системного звука* → включить **Auris** и перезапустить приложение. В Auris статус и кнопка перехода — в **Настройки → Запись**. Записывается только звук, не экран; звуки самого Auris не попадают в запись. Без разрешения Auris всё равно пишет микрофон и предупреждает об этом.
...
- **Запись** — две дорожки 16 кГц моно. Windows: микрофон (event-режим WASAPI) и системный звук (loopback, опрос). macOS: микрофон через CoreAudio, системный звук через ScreenCaptureKit (без звуков самого Auris).
